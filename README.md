...
or when the agent is enabled on a different pin — run `aleph agents pull` first
(it fetches files and stamps `versionId`). New agents (GET 404) may publish
without `versionId`; a successful push/sync stamps it.
